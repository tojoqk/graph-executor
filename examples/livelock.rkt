...
(module+ test
  ...
  (check-equal? (find-livelock id-m) (list (journal-entry 'choose "Id")))

  (define tri-m (make-triangle-model))
  (check-equal? (find-livelock tri-m) (list (journal-entry 'choose "C->A")
                                            (journal-entry 'choose "B->C")
                                            (journal-entry 'choose "A->B")))

  (define ρ-m (make-ρ-model))
  (: ρ-terminal-node? (-> Node-Info Boolean))
  (define (ρ-terminal-node? n)
    (symbol=? (node-info-type n) 'terminal))
  (check-equal? (find-livelock ρ-m) (list (journal-entry 'choose "D->B")
