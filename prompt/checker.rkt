...
(: default-checker-prompt-string-values (-> Prompt-Meta (Pairof String (Listof String))))
...
(: default-checker-prompt-integer-values (-> Prompt-Meta (Pairof Integer (Listof Integer))))
...
(: default-checker-prompt-natural-values (-> Prompt-Meta (Pairof Natural (Listof Natural))))
...
(: default-checker-prompt-positive-integer-values (-> Prompt-Meta (Pairof Positive-Integer (Listof Positive-Integer))))
...
(: default-checker-prompt-range-values (-> Prompt-Meta Integer Integer (U (Pairof Integer (Listof Integer))
                                                                          'ascending
                                                                          'descending)))
(define default-checker-prompt-range-values (lambda (_m _from _to) 'ascending))

(: default-checker-prompt-random-values (-> Prompt-Meta Positive-Integer (U (Pairof Natural (Listof Natural))
                                                                            'ascending
                                                                            'descending)))
...
(struct %checker-prompt-config ([string-values : (-> Prompt-Meta (Pairof String (Listof String)))]
                                [integer-values : (-> Prompt-Meta (Pairof Integer (Listof Integer)))]
                                [natural-values : (-> Prompt-Meta (Pairof Natural (Listof Natural)))]
                                [positive-integer-values : (-> Prompt-Meta (Pairof Positive-Integer (Listof Positive-Integer)))]
                                [range-values : (-> Prompt-Meta Integer Integer (U (Pairof Integer (Listof Integer))
                                                                                   'ascending
                                                                                   'descending))]
                                [random-values : (-> Prompt-Meta Positive-Integer (U (Pairof Natural (Listof Natural))
                                                                                     'ascending
                                                                                     'descending))])
  #:type-name Checker-Prompt-Config)

(: checker-prompt-config (-> [#:string-values (Option (-> Prompt-Meta (Pairof String (Listof String))))]
                             [#:integer-values (Option (-> Prompt-Meta (Pairof Integer (Listof Integer))))]
                             [#:natural-values (Option (-> Prompt-Meta (Pairof Natural (Listof Natural))))]
                             [#:positive-integer-values (Option (-> Prompt-Meta (Pairof Positive-Integer (Listof Positive-Integer))))]
                             [#:range-values (Option (-> Prompt-Meta Integer Integer (U (Pairof Integer (Listof Integer))
                                                                                        'ascending
                                                                                        'descending)))]
                             [#:random-values (-> Prompt-Meta Positive-Integer (U (Pairof Natural (Listof Natural))
                                                                                  'ascending
...
(define ((checker-prompt amb config) meta op)
  (case (car op)
    [(choose) (checker-choose amb op)]
    [(integer) (values (list->amb amb exact-integer? ((%checker-prompt-config-integer-values config) meta)) '())]
    [(natural) (values (list->amb amb natural? ((%checker-prompt-config-natural-values config) meta)) '())]
    [(positive-integer) (values (list->amb amb exact-positive-integer? ((%checker-prompt-config-positive-integer-values config) meta)) '())]
    [(string) (values (list->amb amb string? ((%checker-prompt-config-string-values config) meta)) '())]
    [(range) (checker-range amb config meta op)]
    [(random) (checker-random amb config meta op)]))
...
(define (checker-range amb config meta op)
  (let ([from (second op)]
        [to : Integer (third op)])
    (unless (<= from to)
      (error 'prompt "invalid range ~a...~a" from to))
    (values (let ([vals ((%checker-prompt-config-range-values config) meta from to)])
              (if (pair? vals)
...
(: checker-random (-> (-> (-> Prompt-Value) * Prompt-Value) Checker-Prompt-Config Prompt-Meta (List 'random Positive-Integer) (Values Natural Prompt-Attributes)))
(define (checker-random amb config meta op)
  (let ([n (second op)])
    (values (let ([vals ((%checker-prompt-config-random-values config) meta n)])
              (case vals
